...
import {
  ...
  BarChart3, Wrench, CreditCard, LogOut, Menu, X, User, KeyRound, UserPlus, ChevronUp
} from "lucide-react";
  ...
  return (
    <>
      ...
      >
        ...
        <div className="border-t border-sidebar-border flex flex-col">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-3 px-5 h-12 text-sidebar-foreground hover:bg-sidebar-accent/50 transition-colors text-sm outline-none">
                <User className="w-4 h-4" />
                <span className="flex-1 text-left">User Profile</span>
                <ChevronUp className="w-3 h-3 opacity-50" />
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-48 bg-[#0B1222] border-slate-800" side="right" align="end">
              <DropdownMenuLabel className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">User Account</DropdownMenuLabel>
              <DropdownMenuSeparator className="bg-slate-800" />
              <DropdownMenuItem
                className="text-sm cursor-pointer hover:bg-sidebar-accent focus:bg-sidebar-accent"
                onSelect={() => setIsChangePasswordModalOpen(true)}
              >
                <KeyRound className="w-4 h-4 mr-2" />
                Change Password
              </DropdownMenuItem>
              <NavLink to="/register" onClick={() => setOpen(false)}>
                <DropdownMenuItem className="text-sm cursor-pointer hover:bg-sidebar-accent focus:bg-sidebar-accent">
