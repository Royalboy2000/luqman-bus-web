...
export default function ExpensesPage() {
  ...
  const reload = async () => {
    try { setPayables(await getPayables()); } catch { toast.error('Failed to load payables'); }
  };
  useEffect(() => { reload(); }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await savePayable(form);
      setForm({ payeeName: '', amount: 0, dateTaken: '', dateDue: '', status: 'unpaid' });
      setShowForm(false);
      await reload();
      toast.success('Payable added');
    } catch { toast.error('Failed to save payable'); }
  };

  const markPaid = async (id: string) => {
    try { await updatePayable(id, { status: 'paid' }); await reload(); toast.success('Marked as paid'); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" type="button" onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Add Payable</Button>
      </div>
