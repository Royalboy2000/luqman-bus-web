...
export default function MaintenancePage() {
  ...
  const reload = async () => {
    try {
      const [entries, trucks] = await Promise.all([getMaintenanceEntries(), getTrucks()]);
      setEntries(entries);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await saveMaintenanceEntry({ ...form, cost: form.cost || undefined });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" type="button" onClick={() => setShowForm(true)}><Plus className="w-4 h-4" /> Add Entry</Button>
      </div>
