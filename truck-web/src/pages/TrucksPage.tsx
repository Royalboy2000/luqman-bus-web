...
export default function TrucksPage() {
  ...
  const reload = async () => {
    try { setTrucks(await getTrucks()); } catch { toast.error('Failed to load trucks'); }
  };
  useEffect(() => { reload(); }, []);
  ...
  const handleDelete = async (id: string) => {
    ...
    } catch { toast.error('Failed to delete truck'); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => { setEditing(null); setForm({ registration: '', make: '', model: '', mileage: 0, status: 'active' }); setShowForm(true); setDeleteId(null); }}>
        ...
        <div className="stat-card animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="flex gap-2">
              <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.status} onChange={e => setForm({ ...form, status: e.target.value as 'active' | 'inactive' })}>
                ...
                <td className="text-right">
                  {deleteId === t.id ? (
                    <span className="inline-flex items-center gap-1">
                      ...
                      <button type="button" onClick={() => handleDelete(t.id)} className="text-destructive hover:text-destructive/80 p-1"><Check className="w-3.5 h-3.5" /></button>
                    ...
                    <>
                      <button type="button" onClick={() => handleEdit(t)} className="text-muted-foreground hover:text-foreground p-1"><Pencil className="w-3.5 h-3.5" /></button>
                      <button type="button" onClick={() => setDeleteId(t.id)} className="text-muted-foreground hover:text-destructive p-1 ml-1"><Trash2 className="w-3.5 h-3.5" /></button>
                    </>
                  )}
                </td>
              </tr>
