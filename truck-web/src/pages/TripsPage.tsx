...
import { Plus, X, ChevronDown, ChevronUp, Fuel, MapPin, DollarSign } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <FuelEntryForm
        trucks={trucks}
        onSave={(entry) => {
          toast.success(`Fuel entry added for ${getTruckReg(entry.truckId)}`);
        }}
      />
            ...
            {expanded === t.id && (
              <div className="mt-4 pt-4 border-t border-border animate-fade-in">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-8">
                  <div><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Driver</span>{getDriverName(t.driverId)}</div>
                  <div><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Tanman</span>{t.tanmanId ? getDriverName(t.tanmanId) : '—'}</div>
                  <div><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Total Fuel</span><span className="font-mono">KSh {fmt(t.outbound.fuelCost + (t.returnLeg?.fuelCost || 0))}</span></div>
                  <div><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Trip Pay</span><span className="font-mono">KSh {fmt(t.tripPay)}</span></div>
                  <div><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Total Expenses</span><span className="font-mono">KSh {fmt(calcTripExpenses(t))}</span></div>
                  <div><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Total Mileage</span><span className="font-mono">{(t.outbound.mileage + (t.returnLeg?.mileage || 0)).toLocaleString()} km</span></div>
                  {t.outbound.spareParts && <div className="col-span-2"><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Outbound Parts</span>{t.outbound.spareParts}</div>}
                  {t.returnLeg?.spareParts && <div className="col-span-2"><span className="text-muted-foreground text-xs block font-bold uppercase tracking-wider mb-1">Return Parts</span>{t.returnLeg.spareParts}</div>}
                </div>

                <div className="space-y-4">
                  <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Fuel className="w-4 h-4 text-primary" />
                    Journey Timeline - Fuel Stops
                  </h3>

                  <FuelStopCard
                    location={t.outbound.fuelLocation || "Outbound Stop"}
                    liters={t.outbound.litresPurchased || 0}
                    pricePerLiter={t.outbound.fuelPricePerLitre || 0}
                    totalCost={t.outbound.fuelCost}
                    odometer={undefined}
                  />

                  {t.returnLeg && t.returnLeg.fuelCost > 0 && (
                    <FuelStopCard
                      location={t.returnLeg.fuelLocation || "Return Stop"}
                      liters={t.returnLeg.litresPurchased || 0}
                      pricePerLiter={t.returnLeg.fuelPricePerLitre || 0}
                      totalCost={t.returnLeg.fuelCost}
                      odometer={undefined}
                    />
                  )}
                </div>
