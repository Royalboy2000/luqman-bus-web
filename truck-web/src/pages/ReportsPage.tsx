...
import { getTrips, getTrucks, getDrivers, getClients, getAdvances, getPayables, calcTripRevenue, calcTripExpenses, calcTripProfit, Trip } from "@/lib/store";
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    Promise.all([getTrips(), getTrucks(), getDrivers(), getClients(), getAdvances(), getPayables()]).then(
      ([trips, trucks, drivers, clients, advances, payables]) => {
        setTrips(trips); setTrucks(trucks); setDrivers(drivers);
        setClients(clients); setAdvances(advances); setPayables(payables);
      }
    );
  }, []);
          ...
          <table className="data-table">
            ...
            <tbody>
              {drivers.map(d => {
                ...
                const adv = advances.filter((a: any) => a.personId === d.id).reduce((s: number, a: any) => s + a.amount, 0);
